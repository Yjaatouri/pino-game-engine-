...
int main() {
    ...
    while (eng.is_running()) {
        ...
        f32 y = 30;
        tr.begin_frame();
        tr.draw_string(font, "Hello, Pino Engine!", 30, y, 1.0f);
        tr.flush(1,1,1,1); y += 22;

        tr.begin_frame();
        tr.draw_string(font, "ABCDEFGHIJKLMNOPQRSTUVWXYZ", 30, y, 1.0f);
        tr.flush(0,1,1,1); y += 22;

        tr.begin_frame();
        tr.draw_string(font, "abcdefghijklmnopqrstuvwxyz", 30, y, 1.0f);
        tr.flush(0,1,0,1); y += 22;

        tr.begin_frame();
        tr.draw_string(font, "0123456789", 30, y, 1.0f);
        tr.flush(1,1,0,1); y += 22;

        tr.begin_frame();
        tr.draw_string(font, "!@#$%^&*()_+-=[]{}|;:',.<>?/~`", 30, y, 1.0f);
        tr.flush(1,0,1,1); y += 22;
        y += 10;

        tr.begin_frame();
        tr.draw_string(font, "Scale 0.5x", 30, y, 0.5f);
        tr.flush(0.7f,0.7f,1,1); y += 12;

        tr.begin_frame();
        tr.draw_string(font, "Scale 2.0x", 30, y, 2.0f);
        tr.flush(1,0.5f,0.5f,1); y += 32;

        tr.begin_frame();
        tr.draw_string(font, "ASCII 32-126:", 30, y, 1.0f);
        tr.flush(0.7f,0.7f,0.7f,1); y += 20;
        ...
        full[93] = 0;
        tr.begin_frame();
        tr.draw_string(font, full, 30, y, 0.7f);
        tr.flush(0.6f,0.6f,0.6f,1); y += 16;
        ...
        tr.begin_frame();
        tr.draw_string(font, fps_buf, static_cast<f32>(fw - 200), static_cast<f32>(fh - 24), 0.8f);
        tr.flush(0.4f,0.8f,0.4f,1);

        // Validation results banner
        tr.begin_frame();
        tr.draw_string(font, errs == 0 ? "ALL TESTS PASSED" : "SOME TESTS FAILED",
                       30.0f, static_cast<f32>(fh - 24), 0.9f);
        tr.flush(errs == 0 ? 0.2f : 1.0f,
                 errs == 0 ? 0.9f : 0.2f,
                 errs == 0 ? 0.2f : 0.2f, 1);
