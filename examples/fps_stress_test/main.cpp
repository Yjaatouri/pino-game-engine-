...
struct WeaponComponent {
    ...
    float reload_time = 2, reload_timer = 0;
...
static glm::vec3 rand_pos(float margin = 2) {
    float h = ARENA_HALF - margin;
    return {(rng()*2-1)*h, ENTITY_R, (rng()*2-1)*h};
}

static void clamp_pos(glm::vec3& p, float r) {
...
bool FpsGame::init() {
    ...
    // Pillars
    for (int i = 0; i < 6; ++i) {
        glm::vec3 p = rand_pos(3); p.y = 1.5f;
        EntityId e = m_world.create_entity();
        auto& sg = m_world.scene().scene_graph();
        sg.attach(e);
        sg.set_position(e, p);
    ...
    PINO_INFO("── FPS Stress Test ──");
    PINO_INFO("WASD=move Mouse=look LMB=shoot R=reload");
    PINO_INFO("F3=debug F7=spawn+10 ESC=exit");
    return true;
}
...
void FpsGame::do_shoot(const Ray& ray) {
    ...
    if (hp->hp <= 0) { m_kills++; m_score += 100; kill_entity(target); }
}

// ── Save/Load ─────────────────────────────────────────────────────
void FpsGame::save_game() {
    PINO_INFO("[SAVE] Kills=%d Entities=%u Score=%d", m_kills, m_world.entity_count(), m_score);
}
void FpsGame::load_game() {
    PINO_INFO("[LOAD] placeholder — full ECS serialization via SaveGameSerializer");
}
...
void FpsGame::update(f32 dt) {
    ...
    if (in.key_pressed(Key::F5)) save_game();
    if (in.key_pressed(Key::F6)) load_game();
    if (in.key_pressed(Key::F7)) { for (int i=0;i<10;++i) spawn_enemy(); m_total_spawned+=10; }
    ...
    if (!m_alive) {
        ...
        if (m_respawn_timer <= 0) {
            ...
            m_fps.set_position({0, ENTITY_R, 0});
            m_cam.set_position({0, ENTITY_R, 0});
        }
    ...
    m_fps.update(in, dt);
    glm::vec3 ppos = m_cam.position(); ppos.y = ENTITY_R;
    clamp_pos(ppos, ENTITY_R);
    if (sg.has(m_pid)) sg.set_position(m_pid, ppos);

    // Audio listener
    au.set_listener_position(m_cam.position());
    au.set_listener_orientation(glm::normalize(m_cam.target()-m_cam.position()), {0,1,0});
    ...
    if (wp) {
        ...
        if (in.is_mouse_pressed(MouseButton::Left) && !wp->reloading) {
            if (m_game_time - wp->last_fire >= wp->fire_rate && wp->ammo > 0) {
                ...
                r.direction = glm::normalize(m_cam.target() - m_cam.position());
                do_shoot(r);
                au.play_one_shot("audio/test_tone.wav", 0.4f);
            }
        }
    ...
    m_pool_ai.each([&](EntityId e, EnemyAIComponent& ai) {
        if (!sg.has(e) || !sg.has(m_pid)) return;
        ...
        if (dist < ai.attack_range && ai.attack_timer <= 0) {
            ...
            if (php) {
                ...
                au.play_one_shot("audio/test.wav", 0.6f);
                if (php->hp <= 0) { kill_entity(m_pid); m_alive = false; m_respawn_timer = RESPAWN_DELAY; }
            }
        }
        auto* rc = m_world.scene().get_component<RenderComponent>(e);
    ...
    m_spawn_timer -= dt;
    if (m_spawn_timer <= 0 && alive < MAX_ENEMIES) {
...
void FpsGame::draw_hud(i32 w, i32 h) {
    ...
    if (php && m_alive) {
        ...
        for (int i = 0; i < 20; ++i) buf[13+i] = (i < bars) ? '|' : '.';
        buf[33] = ']'; buf[34] = 0;
        float r = 1-pct, g = pct;
        m_txt.draw_text(m_font, buf, w*0.5f-120, h-40, 0.35f, r, g, 0.05f, 1);
    }
    ...
    if (wp && m_alive) {
        char buf[64];
        if (wp->reloading) std::snprintf(buf, sizeof(buf), "RELOADING...");
        else std::snprintf(buf, sizeof(buf), "Ammo: %d/%d", wp->ammo, wp->max_ammo);
    ...
    // Stats
    {
        ...
        std::snprintf(buf, sizeof(buf), "Score: %d  Kills: %d  Enemies: %u  Entities: %u",
                      m_score, m_kills, ec, m_world.entity_count());
        m_txt.draw_text(m_font, buf, 20, 20, 0.3f, 0.8f, 0.9f, 1, 1);
    }
    ...
    if (!m_alive) {
        ...
        m_txt.draw_text(m_font, buf, w*0.5f-120, h*0.5f, 0.6f, 1,0.2f,0.2f,1);
    }
...
void FpsGame::shutdown() { m_world.destroy(); m_txt.destroy(); m_font.destroy(); m_ground.destroy(); }
...
int main(int, char**) {
    Engine engine;
    EngineConfig cfg;
    cfg.app_title = "Pino FPS Stress Test";
